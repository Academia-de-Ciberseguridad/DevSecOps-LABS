...
t = u.personal_access_tokens.create!(scopes: ['api','read_repository','write_repository','create_runner'], name: 'lab-automation', expires_at: 365.days.from_now)
puts t.token
" 2>/dev/null | grep -oE 'glpat-[A-Za-z0-9._-]+' | head -1)   # incluye '.' (formato routable glpat-xxx.01.yyy)
